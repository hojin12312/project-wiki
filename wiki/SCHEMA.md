...
- `wiki-language` is the canonical language for newly written and substantively rewritten wiki prose. New wikis default to English: the wiki's primary reader is a future agent session, so existing docs in another language are not by themselves a reason to choose that language. Use another language only when the user asks for it, a repository instruction requires it, or the project genuinely demands the original language.
- An existing wiki's `wiki-language` changes only with the user's approval, never as part of an ordinary update. During an approved incremental language migration, existing pages in another language may remain in that language; they are converted when substantively edited or when promoted into the bootstrap set (index, overview, current). `wiki-language: en` does not require every legacy page to be English already.
- Source-language text may remain in any wiki when the exact wording carries information — user quotations, requirements whose phrasing matters, UI strings, error messages, command output, legal or domain-specific terminology — normally beside a short English summary.
...
- Language: write new prose in `wiki-language`. During an approved incremental language migration, a page that still carries another language is converted only when the run substantively rewrites it or promotes it into the bootstrap set; a cosmetic edit (typo, link, date) never triggers conversion, `archive/` and old `log.md` entries are left alone, and the existence of non-English pages is never by itself a reason to edit (`core/protocol.md` §10).
