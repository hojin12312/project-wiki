...
- Git 안전 절차, instruction 파일에 안내를 넣는 방법, 보고 형식 같은 절차는 `core/protocol.md`에서 정합니다.
...
- 버전은 두 종류입니다. `VERSION`은 skill package의 버전이고, `core/SCHEMA_VERSION`은 SCHEMA 정책의 버전입니다. 이미 만들어진 Wiki와 비교할 때는 SCHEMA 정책 버전만 사용합니다.
